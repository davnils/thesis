...
The first main improvement is increased efficiency since faulty solar panels are detected and can be replaced sooner.
...
Both of these share the property of being constructed around a single pn-junction, i.e. the connection between two differently doped silicon areas.
The incoming photons may be absorbed by the silicon in which case an electron-hole pair is created~\cite{Zhao2010thesis}, since the electron is pushed to an outer layer by the absorbed energy.
...
There are, however, also some extra components involved in the module circuitry.
...
This is highly relevant since studies in fault detection typically consider a specific topology and the results may not always generalize to the distributed case.
...
In addition to seasonal and geographical dependencies, the power output is also proportional to the number of solar panels in the system and is affected by the type of each panel.
...
In addition, it is not applicable to passive studies of systems since they can be assumed to deliver output at the MPP.
...
All the surveyed methods exhibit different properties, but the paper concludes that the Hampel identifier and Boxplot rule are suitable for fault detection.
...
The requirement of access to panel temperature is however limiting in the context of this report.
...
Finally, the resulting voltage and current vectors are stored together with the input temperature vector.
...
The one-diode model can be applied onto measurements of the free variables in order to find the corresponding parameters.
...
The results will then only establish an upper bound in performance for this method.
...
For example, consider the case of a solar installation being subject to nearly identical irradiation during two consecutive days.
Given that the environment temperature remains similar, this will lead to very similar samples being taken along the curves of both days.
...
Hence this vector can be analyzed in order to draw conclusions regarding changes in the measurements taken.
...
Smaller threshold values will produce more accurate comparisons but have the inherent limitation of limiting the number of data points retrieved, eventually resulting in no data points.
...
Brighter areas have a higher density of solar occurrences.
...
The approach taken here is to study clusters of faults.
...
The first one is $\epsilon$ which describes the minimum distance between two points in the same cluster.
...
Finally, as demonstrated by figure \ref{fig:solar-plane-plot}, the non-uniform distribution of solar paths implies that 
...
This is shown in the figures below where the classification rate has been plotted as a function of the fault percentage.
...
These values were chosen such that the requirement of no false-positives was satisfied.
...
Based on the literature surveyed, this situation presents new challenges in detecting faults.
...
A limitation of this method is that only a single panel fault can be detected at each point in time.
...
The method is computationally efficient since the complexity remains constant in the number of samples acquired.
This implies that an embedded implementation could be developed, which would be placed at the solar installation site.
Since there is no dependence on external information, such as solar position, this would also be applicable to sites without network connectivity.

Future work could include improving the sensitivity to less significant faults.
More advanced statistical methods and hypothesis testing using confidence intervals could improve the performance.
...
This is followed by calculating the degradation ratio in both voltage and current samples respectively.
...
Hence it can be concluded that there is a balance between the amount of degradation that is reliably detectable and the amount of valid points.
...
The results demonstrate that the cluster of shading faults was successfully found, but also demonstrate the need for application-specific tuning.
