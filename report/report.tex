...
based on the one-diode model introduced previously (equation \ref{}).
...
Another statistical approach is taken in \cite{Zhao2013outlier} which is centered around outlier detection.
%TODO

In the case of given knowledge surrounding faults it is possible to apply supervised learning, e.g. \cite{Zhao2012tree}.
It discusses TODO.

\cite{Zhao2013graph} extends this result to the case of semi-supervised learning, where only a small amount of reference data is available from the start.
...
Finally, \cite{Kang2012} takes the approach of using a Kalman filter on TODO.

There has also been research into locating the faulty panel within a string \cite{Lin2012}, which is considered less applicable when individual solar module measurements are available.
